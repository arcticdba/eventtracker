...
export function StatisticsLab({ events, eventSeries, sessions, submissions, dateFormat }: Props) {
  ...
  const visitedCountries = useMemo(() => {
    const grouped = new Map<string, Event[]>()
    selectedEvents.filter(event => !event.remote && event.country && !isOnlineLocation(event.country)).forEach(event => {
      grouped.set(event.country, [...(grouped.get(event.country) ?? []), event])
    })
    return [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b))
  }, [selectedEvents])
  const outcomeEvents = expandedOutcome
  ...
  return <div className="space-y-5 pb-8">
    ...
    <div className="grid gap-5 lg:grid-cols-3"><section className="ui-card lg:col-span-2"><h3 className="text-lg font-semibold">Performance by event year</h3><p className="mb-4 text-sm text-gray-500">Every event is counted once in its start year.</p><div className="space-y-3">{yearStats.map(item => <button key={item.year} onClick={() => setYear(String(item.year))} className="grid w-full grid-cols-[3rem_1fr_5rem] items-center gap-3 text-left"><span className="text-sm font-medium">{item.year}</span><div className="h-7 overflow-hidden rounded bg-gray-100"><div className="flex h-full" style={{ width: `${item.total / maxYearTotal * 100}%` }}><div className="bg-emerald-500" style={{ width: `${item.total ? item.accepted / item.total * 100 : 0}%` }} /><div className="bg-rose-400" style={{ width: `${item.total ? item.rejected / item.total * 100 : 0}%` }} /><div className="bg-amber-300" style={{ width: `${item.total ? item.pending / item.total * 100 : 0}%` }} /><div className="flex-1 bg-gray-300" /></div></div><span className="text-right text-sm font-semibold">{item.acceptance === null ? '—' : `${item.acceptance}%`}</span></button>)}</div></section><section className="ui-card"><h3 className="text-lg font-semibold">Speaking footprint</h3><p className="mb-4 text-sm text-gray-500">Based on accepted events in this view.</p><div className="grid grid-cols-2 gap-3"><SmallMetric label="Unique events" value={selectedEvents.length} /><SmallMetric label="Countries" value={countries} active={expandedFootprint === "countries"} onClick={() => setExpandedFootprint(expandedFootprint === "countries" ? null : "countries")} /><SmallMetric label="Cities" value={cities} active={expandedFootprint === "cities"} onClick={() => setExpandedFootprint(expandedFootprint === "cities" ? null : "cities")} /><SmallMetric label="Remote" value={remoteEngagements} /></div><div className={expandedFootprint === "countries" ? "mt-4 border-t pt-4" : "hidden"}><h4 className="mb-2 text-sm font-semibold">Countries visited ({visitedCountries.length})</h4>{visitedCountries.length ? <div className="flex max-h-44 flex-wrap content-start gap-2 overflow-y-auto">{visitedCountries.map(([country, countryEvents]) => <span key={country} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-700" title={countryEvents.map(event => `${event.name} (${formatDate(event.dateStart, dateFormat)})`).join("\n")}>{getCountryFlagOrEmpty(country)}{getCountryFlagOrEmpty(country) && " "}{country} {countryEvents.length > 1 && <strong>×{countryEvents.length}</strong>}</span>)}</div> : <p className="text-sm text-slate-400">No in-person countries in this view.</p>}</div><div className={expandedFootprint === "cities" ? "mt-4 border-t pt-4" : "hidden"}><h4 className="mb-2 text-sm font-semibold">Cities visited ({visitedCities.length})</h4>{visitedCities.length ? <div className="flex max-h-44 flex-wrap content-start gap-2 overflow-y-auto">{visitedCities.map(([location, locationEvents]) => { const country = locationEvents[0]?.country ?? ''; return <span key={location} className="rounded-full bg-gray-100 px-2.5 py-1 text-xs text-gray-700" title={locationEvents.map(event => `${event.name} (${formatDate(event.dateStart, dateFormat)})`).join('\n')}>{getCountryFlagOrEmpty(country)}{getCountryFlagOrEmpty(country) && ' '}{location} {locationEvents.length > 1 && <strong>×{locationEvents.length}</strong>}</span> })}</div> : <p className="text-sm text-gray-400">No in-person cities in this view.</p>}</div></section></div>
  </div>
}

function Metric({ label, value, detail, color = 'text-gray-900' }: { label: string; value: string | number; detail: string; color?: string }) { return <div className="ui-card"><div className="text-xs font-semibold uppercase tracking-wide text-gray-500">{label}</div><div className={`mt-1 text-3xl font-bold ${color}`}>{value}</div><div className="mt-1 text-xs text-gray-500">{detail}</div></div> }
function SmallMetric({ label, value, active = false, onClick }: { label: string; value: number; active?: boolean; onClick?: () => void }) {
  const content = <><div className="text-2xl font-bold text-blue-700">{value}</div><div className="flex items-center justify-between gap-1 text-xs text-slate-500"><span>{label}</span>{onClick && <span aria-hidden="true">{active ? '▲' : '▼'}</span>}</div></>
  return onClick
    ? <button type="button" onClick={onClick} aria-expanded={active} className={`rounded-lg border p-3 text-left transition ${active ? 'border-blue-300 bg-blue-50' : 'border-transparent bg-slate-50 hover:border-slate-300'}`}>{content}</button>
    : <div className="rounded-lg border border-transparent bg-slate-50 p-3">{content}</div>
}
function OutcomeBadge({ outcome }: { outcome: EventOutcome }) {
  return <span className={`inline-flex w-fit shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold uppercase tracking-wide ring-1 ${submissionBadgeStyles[outcome]}`}><span className={`h-2 w-2 rounded-full ${submissionDotStyles[outcome]}`} />{outcomeLabels[outcome]}</span>
}
function ChartLegend() {
  return <div className="mb-3 flex flex-wrap gap-3 text-xs text-slate-500"><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-emerald-500" />Accepted</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-rose-400" />Rejected</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-amber-300" />Pending</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-gray-300" />Other</span></div>
}
function GroupBreakdown({ title, description, stats }: { title: string; description: string; stats: Array<{ name: string; total: number; accepted: number; rejected: number; pending: number; acceptance: number | null }> }) {
  ...
  return <section className="ui-card"><h3 className="ui-section-title">{title}</h3><p className="ui-supporting-text mb-3">{description}</p><ChartLegend />{stats.length ? <div className="space-y-3">{stats.map(item => <div key={item.name}><div className="mb-1 flex items-baseline justify-between gap-3"><span className="font-medium text-slate-800">{item.name}</span><span className="text-sm font-bold">{item.acceptance === null ? '—' : `${item.acceptance}%`}</span></div><div className="h-5 overflow-hidden rounded-full bg-slate-100"><div className="flex h-full" style={{ width: `${item.total / maxTotal * 100}%` }}><div className="bg-emerald-500" style={{ width: `${item.total ? item.accepted / item.total * 100 : 0}%` }} /><div className="bg-rose-400" style={{ width: `${item.total ? item.rejected / item.total * 100 : 0}%` }} /><div className="bg-amber-300" style={{ width: `${item.total ? item.pending / item.total * 100 : 0}%` }} /><div className="flex-1 bg-gray-300" /></div></div><div className="mt-1 text-xs text-slate-500">{item.total} event{item.total === 1 ? '' : 's'} · {item.accepted} accepted · {item.rejected} rejected · {item.pending} pending</div></div>)}</div> : <div className="py-6 text-center text-sm text-slate-500">No events match these filters.</div>}</section>
}
